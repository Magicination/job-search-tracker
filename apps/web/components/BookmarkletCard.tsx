 ...
 * прямой запрос к api.hh.ru (тот заблокирован анти-бот защитой). APP_URL
 * вычисляется из window.location.origin в момент установки — работает
 * "из коробки" на любом домене деплоя без пропа/ручной правки кода.
 * Если структурного поля "Опыт" на странице нет — пробуем угадать его из
 * текста описания вакансии (jd.description) простыми регулярками.
 */
