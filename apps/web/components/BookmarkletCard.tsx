...
function buildBookmarkletCode(appUrl: string): string {
  return `javascript:(function(){var APP_URL='${appUrl}';function getJsonLd(){var s=document.querySelectorAll('script[type="application/ld+json"]');for(var i=0;i<s.length;i++){try{var d=JSON.parse(s[i].textContent);var list=Array.isArray(d)?d:[d];for(var j=0;j<list.length;j++){if(list[j]&&list[j]['@type']==='JobPosting')return list[j];}}catch(e){}}return null;}function text(sel){var el=document.querySelector(sel);return el?el.textContent.trim():'';}function stripHtml(html){var div=document.createElement('div');div.innerHTML=html;return(div.textContent||div.innerText||'').replace(/\\s+/g,' ').trim();}function guessExperience(desc){if(!desc)return'';var d=desc.toLowerCase();if(/без опыта|не требуется|no experience|entry.level/.test(d))return'Без опыта';var range=d.match(/(\\d+)\\s*[-–]\\s*(\\d+)\\s*(?:лет|года|год|years?)/);if(range)return range[1]+'–'+range[2]+' лет';var single=d.match(/от\\s*(\\d+)\\s*(?:лет|года|год|years?)/);if(single)return'от '+single[1]+' лет';return'';}var jd=getJsonLd()||{};var company=(jd.hiringOrganization&&jd.hiringOrganization.name)||text('[data-qa="vacancy-company-name"]')||'';var companyUrl='';if(jd.hiringOrganization){companyUrl=jd.hiringOrganization.sameAs||jd.hiringOrganization.url||'';}var role=jd.title||text('[data-qa="vacancy-title"]')||document.title||'';var salary='';if(jd.baseSalary&&jd.baseSalary.value){var v=jd.baseSalary.value;var cur=jd.baseSalary.currency||'';if(v.minValue&&v.maxValue)salary=v.minValue+'–'+v.maxValue+' '+cur;else if(v.minValue)salary='от '+v.minValue+' '+cur;else if(v.maxValue)salary='до '+v.maxValue+' '+cur;}if(!salary)salary=text('[data-qa="vacancy-salary"]');var descriptionText=jd.description?stripHtml(jd.description):text('[data-qa="vacancy-description"]');var experience=text('[data-qa="vacancy-experience"]');if(!experience&&descriptionText)experience=guessExperience(descriptionText);var description=descriptionText?descriptionText.slice(0,1500):'';var url=window.location.href;var params=new URLSearchParams({company:company,companyUrl:companyUrl,role:role,salary:salary,experience:experience,description:description,url:url});window.open(APP_URL+'/add?'+params.toString(),'_blank');})();`;
}
